...
static thread_local LinuxSampler::Sampler * sampler = NULL;

class GwionOutput : public LinuxSampler::AudioOutputDevice
{
public:
  GwionOutput(std::map<String,LinuxSampler::DeviceCreationParameter*> param, m_float fs, size_t size) :
...
class myLinuxSampler : public GwionOutput
{
public:
  myLinuxSampler(std::map<String,LinuxSampler::DeviceCreationParameter*> param, m_float fs) : GwionOutput(param, fs, 512), count(0)
  {
  ...
  void tick(m_float* l, m_float* r) {
    ...
    *r = Channel(1)->Buffer()[count%size];
	...
	void load(std::string s, int NUSED instrument = 0) {
...
static CTOR(linuxsampler_ctor) {
  ...
  myLinuxSampler* ls = *(myLinuxSampler**)(o->data +SZ_INT) = new 
...
static DTOR(linuxsampler_dtor) {
  myLinuxSampler * ls_obj = *(myLinuxSampler**) (o->data + SZ_INT);
  delete ls_obj;
}
...
GWION_IMPORT(LinuxSampler) {
  GWI_B(gwi_class_ini(gwi, "LinuxSampler", "UGen"));
  ...
  GWI_B(gwi_func_end(gwi, (f_xfun)linuxsampler_load, ae_flag_none));
  gwi_func_ini(gwi, "void", "load");
    gwi_func_arg(gwi, "string", "filename");
    gwi_func_arg(gwi, "int", "index");
  GWI_B(gwi_func_end(gwi, linuxsampler_load_instrument, ae_flag_none));
  gwi_func_ini(gwi, "int", "noteOn");
    gwi_func_arg(gwi, "int", "note");
    gwi_func_arg(gwi, "int", "velocity");
  GWI_B(gwi_func_end(gwi, linuxsampler_noteOn, ae_flag_none));
  gwi_func_ini(gwi, "int", "noteOff");
    gwi_func_arg(gwi, "int", "note");
    gwi_func_arg(gwi, "int", "velocity");
  GWI_B(gwi_func_end(gwi, linuxsampler_noteOff, ae_flag_none));
  gwi_func_ini(gwi, "void", "noteOff");
    gwi_func_arg(gwi, "int", "pitch");
    gwi_func_arg(gwi, "int", "note");
  GWI_B(gwi_func_end(gwi, linuxsampler_pitchbend, ae_flag_none));
  gwi_func_ini(gwi, "int", "status");
  GWI_B(gwi_func_end(gwi, linuxsampler_status, ae_flag_none));
  gwi_func_ini(gwi, "float", "gain");
  GWI_B(gwi_func_end(gwi, linuxsampler_getgain, ae_flag_none));
  gwi_func_ini(gwi, "float", "gain");
    gwi_func_arg(gwi, "float", "f");
  GWI_B(gwi_func_end(gwi,  linuxsampler_setgain, ae_flag_none));
  gwi_func_ini(gwi, "float", "pan");
  GWI_B(gwi_func_end(gwi, linuxsampler_getpan, ae_flag_none));
  gwi_func_ini(gwi, "float", "pan");
    gwi_func_arg(gwi, "float", "f");
  GWI_B(gwi_func_end(gwi, linuxsampler_setpan, ae_flag_none));
