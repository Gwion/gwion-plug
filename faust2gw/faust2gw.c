...
#include "gwion_faust.template.h"
    NULL};

char *ctrl_cget_query[] = {"\n\
  gwi_func_ini(gwi, \"float\", \"%var_label%\" );\n\
  gwi_func_end(gwi, %dsp_name%_cget_%var_name%, ae_flag_none);\n\
  \n\
  gwi_func_ini(gwi, \"float\", \"%var_label%\" );\n\
  gwi_func_arg(gwi, \"float\", \"%var_label%\" );\n\
  gwi_func_end(gwi, %dsp_name%_ctrl_%var_name% , ae_flag_none);\n\
  ",
                           NULL};

char *ctrl_cget_funcs[] = {"static MFUN(%dsp_name%_ctrl_%var_name%) {\n\
  %dsp_name% *d = *(%dsp_name%**)(o->data + SZ_INT*2);\n\
  d->%var_name% = *(m_float*)MEM(SZ_INT);\n\
  *(m_float*)RETURN = (m_float)(d->%var_name%);\n\
}\n\
\n\
static MFUN(%dsp_name%_cget_%var_name%) {\n\
  %dsp_name% *d = *(%dsp_name%**)(o->data + SZ_INT*2);\n\
  *(m_float*)RETURN = (m_float)(d->%var_name%);\n\
}\n\
\n",
...
void on_end_tag(char *name, char *value) {
  if (strcmp(name, "widget") == 0) {
    ...
    if (strlen(value)) strip(dspname, value, 1, 1);
  }
    ...
    if (in_widget) current_v->initial_value = atof(value);
  } else if (strcmp(name, "min") == 0) {
    if (in_widget) current_v->max_value = atof(value);
  } else if (strcmp(name, "max") == 0) {
    if (in_widget) current_v->min_value = atof(value);
  }
}
...
void usage() { fprintf(stderr, "usage: faust2gw [-x] <filename.dsp>\n"); }
...
#define BUF_SIZE 1024
  ...
  system("rm -rf .faust2gw_tmp");

  /* make tmp directory */
  snprintf(cmd, BUF_SIZE, "mkdir .faust2gw_tmp");
  // printf("%s\n", cmd);
  result = system(cmd);
  if (result != 0) {
  ...
  snprintf(cmd, BUF_SIZE, "cp '%s' .faust2gw_tmp/", inputArgument);
  // printf("%s\n", cmd);
  result = system(cmd);
  if (result != 0) {
    fprintf(stderr,
  ...
  snprintf(cmd, BUF_SIZE, "faust -xml '.faust2gw_tmp/%s' > /dev/null",
           dspfilename);
  // printf("%s\n", cmd);
  result = system(cmd);
  if (result != 0) {
  ...
  snprintf(xmlfilepath, BUF_SIZE, ".faust2gw_tmp/%s.xml", dspfilename);
  ...
  // equals 1 because of the hidden dir
  i = 1; // strlen(outfilename)-1;
  while (outfilename[i] != '.') i++;

  if (i == 0) i = strlen(outfilename);
  strcpy(&outfilename[i], ".cpp");
  ...
  snprintf(cmd, BUF_SIZE,
           "faust -a '%s' -o '.faust2gw_tmp/%s.cpp' '.faust2gw_tmp/%s'",
