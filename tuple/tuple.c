...
static OP_CHECK(opck_at_object_tuple) {
  ...
  if(tuple_match(env, bin->lhs->info->type, bin->rhs->info->type) < 0)
    return env->gwion->type[et_null];
...
static OP_CHECK(opck_cast_tuple_object) {
  ...
  if(tuple_match(env, cast->exp->info->type, exp_self(cast)->info->type) < 0)
    return env->gwion->type[et_null];
...
static OP_EMIT(opem_at_tuple_object) {
  const Exp_Binary *bin = (Exp_Binary*)data;
  // some of those could be resolved statically
  const Instr instr = emit_add_instr(emit, Tuple2Object);
...
//      loc_cpy(env->gwion->mp, pos));
  Type_Decl *td = new_type_decl(env->gwion->mp, tsym, loc_cpy(env->gwion->mp, pos));
...
//      loc_cpy(env->gwion->mp, pos));
  Type_Decl *td = new_type_decl(env->gwion->mp, insert_symbol(TUPLE_NAME), loc_cpy(env->gwion->mp, pos));
...
//      e->d.exp_decl.td = new_type_decl(env->gwion->mp, new_id_list(env->gwion->mp, decl, loc_cpy(env->gwion->mp, e->pos)));
      ...
      e->info->type = env->gwion->type[et_null];
    }
    e = e->next;
  }
...
static ANN Type scan_tuple(const Env env, const Type_Decl *td) {
  ...
  do {
    const Type t = tl->td->xid != insert_symbol("_") ?
...
GWION_IMPORT(tuple) {
  ...
  SET_FLAG(t_tuple, check | ae_flag_scan2 | ae_flag_check | ae_flag_emit);
  SET_FLAG(t_tuple, unary | ae_flag_abstract | ae_flag_template);
  ...
  SET_FLAG(t_unpack, check | ae_flag_scan2 | ae_flag_check | ae_flag_emit);
