...
static MFUN(gwbmi_get_pixel) {
  ...
  if(pixel != BMI_PIXEL_INVALID)
    *(bmi_pixel*)RETURN = pixel;
    ...
    xfun_handle(shred, error);
  }
}

static MFUN(gwbmi_overdraw) {
  const M_Object region = *(M_Object*)MEM(SZ_INT);
  const M_Object layer = *(M_Object*)MEM(SZ_INT*2);
  *(m_int*)RETURN = bmi_buffer_overdraw_buffer(GWI_BMI(o), GWI_RECT(region), GWI_BMI(layer));
}
...
static MFUN(gwbmi_create) {
  const m_uint width = *(m_uint*)MEM(SZ_INT);
  const m_uint height = *(m_uint*)MEM(SZ_INT*2);
  const m_uint flags = *(m_uint*)MEM(SZ_INT*3);
  bmi_buffer *buffer = bmi_buffer_new(width, height, flags);
  if(!buffer) {
    xfun_handle(shred, "[{+R}BMI{0}] can't create buffer.");
    return;
  }
